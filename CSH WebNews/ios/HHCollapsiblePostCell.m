...
- (void) createHeaderButton {
    ...
    [self.headerButton setShowsTouchWhenHighlighted:YES];
...
- (void) layoutSubviews {
    dispatch_async(dispatch_queue_create("Cell Loading", 0), ^{
        ...
        actionFrame.size.height = self.frame.size.height;
...
- (NSString*) headerText {
    ...
    NSInteger numberPastMax = self.indentationLevel - MAX_INDENTATION_LEVEL;
    if (numberPastMax > 0) {
